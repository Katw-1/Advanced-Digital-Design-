...
foreach { channel pins } ${vga_pins} {
    for { set bit 0 } { ${bit} < 4 } {incr bit } {
        ...
        set_location_assignment PIN_${pin} -to color.${channel}\[${bit}\]  
        set_instance_assignement -name IO_STANDARD "3.3-V LVTTL" -to color.${channel}\[${bit}\]
    }  
}
set misc_pins{ \
    reset  B8 "3.3 V SCHMITT TRIGGER" \
