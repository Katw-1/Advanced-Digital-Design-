...
class avalon_interface(object):
	...
	def __init__(self, dut):
		...
		if self._read is not None and self._readdata is None:
			raise Exception('read signal but no readdata!')
		else:
			self._read.value = 0

		if self._write is not None and self._writedata is None:
...
#						else None
	...
	async def read(self, addr):
		...
		self._read.value = 0
		await RisingEdge(self._clk)
		data = self._readdata.value.to_unsigned()
		return data
...
async def testbench(dut):
	...
	if get_bits(features, (23, 16)):
		dut._log.info(f'  digits:     {get_bits(features, (15, 8)):2d}')
			...
			if event.type == pygame.QUIT:
				...
				match event.dict['unicode']:
					...
					case 'o' | 'O':
						...
						control_reg = await avalon_mm.read(1)
						control_reg ^= 0b0001
						await avalon_mm.write(1, control_reg)
						dut._log.info(f'lamps toggled ({await avalon_mm.read(1):0{8}x})')
					case 'd' | 'D':
						dut._log.info('toggle decimal')
						control_reg = await avalon_mm.read(1)
						control_reg ^= 0b0010
						await avalon_mm.write(1, control_reg)
						dut._log.info(f'decimal toggled ({await avalon_mm.read(1):0{8}x})')
					case 'b' | 'B':
						dut._log.info('toggle blanking')
						control_reg = await avalon_mm.read(1)
						control_reg ^= 0b0100
						await avalon_mm.write(1, control_reg)
						dut._log.info(f'blanking toggled ({await avalon_mm.read(1):0{8}x})')
					case 's' | 'S':
						dut._log.info('toggle signed')
						control_reg = await avalon_mm.read(1)
						control_reg ^= 0b1000
						await avalon_mm.write(1, control_reg)
						dut._log.info(f'signed toggled ({await avalon_mm.read(1):0{8}x})')
					case 'r' | 'R':
					...
					case 'v' | 'V':
						dut._log.info(f'count ={count}\n'
							  f'number={await avalon_mm.read(0):0{8}x}\n'
							  f'ctrl  ={await avalon_mm.read(1):0{8}x}')
					case other:
						pass
